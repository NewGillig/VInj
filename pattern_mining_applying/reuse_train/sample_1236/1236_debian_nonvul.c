static Image *ReadGIFImage(const ImageInfo *image_info, ExceptionInfo *exception)
{
    ...
    if ((count != 6) || ((LocaleNCompare((char *)magick, "GIF87", 5) != 0) && (LocaleNCompare((char *)magick, "GIF89", 5) != 0)))
        ...
        if (c == (unsigned char)'!')
        {
            ...
            case 0xff:
            {
                ...
                if (ReadBlobBlock(image, header) != 0)
                    loop = LocaleNCompare((char *)header, "NETSCAPE2.0", 11) == 0 ? MagickTrue : MagickFalse;
                if (loop != MagickFalse)
                {
                    while (ReadBlobBlock(image, header) != 0)
                ...
                else
                {
                    ...
                    else if (magick != MagickFalse)
                    {
                        ...
                        (void)FormatLocaleString(name, sizeof(name), "gif:%.11s", header);
