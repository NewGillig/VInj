    ...
    if (!strcmp(name, "flags"))
    {
        ...
        *ret_modern = 1;
        for (i = gcry_sexp_length(l2) - 1; i > 0; i--)
        {
            ...
            else if (n == 5 && !memcmp(s, "pkcs1", 5) && ctx->encoding == PUBKEY_ENC_UNKNOWN)
                ctx->encoding = PUBKEY_ENC_PKCS1;
            else if (n == 4 && !memcmp(s, "oaep", 4) && ctx->encoding == PUBKEY_ENC_UNKNOWN)
        ...
        for (i = 2; (l2 = gcry_sexp_nth(list, i)) != NULL; i++)
        {
            ...
            if (!(n == 9 && !memcmp(s, "hash-algo", 9)) && !(n == 5 && !memcmp(s, "label", 5)) && !(n == 15 && !memcmp(s, "random-override", 15)))
                break;
            gcry_sexp_release(l2);
        }
        if (!l2)
