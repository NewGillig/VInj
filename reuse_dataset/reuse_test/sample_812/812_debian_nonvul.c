static void dumpCreateDB(PGconn *conn)
{
    ...
    if (server_version >= 80400)
        res = executeQuery(conn, "SELECT pg_encoding_to_char(encoding), "
                                 "datcollate, datctype "
                                 "FROM pg_database "
    ...
    for (i = 0; i < PQntuples(res); i++)
    {
        ...
        else if (strcmp(dbtablespace, "pg_default") != 0 && !no_tablespaces)
        {
            ...
            appendPQExpBuffer(buf, "ALTER DATABASE %s SET TABLESPACE %s; \ n ", fdbname, fmtId(dbtablespace));
