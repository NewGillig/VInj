...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in bar:
        ...
        for single_output in beam_outputs:
            ...
            #if SM(None, prediction, ground_truth).ratio() > 0.9:
    ...
    df = pd.DataFrame({"names": [], "raw_predictions": [], "ground_truth": [], 'is_correct':[]})
    df["names"] = names
    df["raw_predictions"] = raw_predictions
    df["ground_truth"] = ground_truths
    df["is_correct"] = accuracy
    df.to_csv("../data/raw_predictions/CodeT5/VulRepair_raw_preds_final_beam1.csv")
...
def main():
    ...
    if args.do_train:
        ...
        pairs=os.listdir('loc_train')
        dataset = {'name': [], 'source': [], 'target': []}
        for i, pair in enumerate(pairs):
            #if i>100:
            #    break
            #try:
            f=open('./loc_train/'+pair+'/'+pair+'_in.c')
            source=f.read()
            source=source.replace(' ', '<S2SV_blank>')
            source=source.replace('!@#$', ' ')
            f.close()
            f=open('./loc_train/'+pair+'/'+pair+'_out.c')
            target=f.read()
    ...
    if args.do_test:
        ...
        tokenizer.add_tokens(["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>"])
        model.resize_token_embeddings(len(tokenizer))
        model.load_state_dict(torch.load('./saved_models/checkpoint-best-loss/model.bin'))
        model.to(args.device)
        pairs=os.listdir('loc_test')
        dataset = {'name': [], 'source': [], 'target': []}
        for i, pair in enumerate(pairs):
            try:
                f=open('./loc_test/'+pair+'/'+pair+'_in.c')
                source=f.read()
                source=source.replace(' ', '<S2SV_blank>')
                source=source.replace('!@#$', ' ')
                f.close()
                f=open('./loc_test/'+pair+'/'+pair+'_out.c')
                target=f.read()
                target=target.replace(' ', '<S2SV_blank>')
                target=target.replace('!@#$', ' ')
                f.close()
